...
import optax
from optax import ScaleByScheduleState
...
class ALIGNIQLLearner(Agent):
    ...
    def save(self, path_dir):
        # 保存 JAX TrainState
        with open(path_dir  + "score_model", 'wb') as f:
            f.write(flax.serialization.to_bytes(self.score_model))
        wandb.save('score_model')
        
        with open(path_dir  + "target_score_model", 'wb') as f:
            f.write(flax.serialization.to_bytes(self.target_score_model))
        wandb.save('target_score_model')
        
        with open(path_dir  + "critic", 'wb') as f:
            f.write(flax.serialization.to_bytes(self.critic))
        wandb.save('critic')
        
        with open(path_dir  + "value", 'wb') as f:
            f.write(flax.serialization.to_bytes(self.value))
        wandb.save('value')
        
        with open(path_dir  + "target_critic", 'wb') as f:
            f.write(flax.serialization.to_bytes(self.target_critic))
        wandb.save('target_critic')
        
    def load(self, path_dir):
        #pdb.set_trace()
        agent = self
        with open(path_dir  + "score_model", 'rb') as f:
            state_dict = flax.serialization.msgpack_restore(f.read())
        opt_state = self.loadUtil(state_dict["opt_state"])
        new_state = ScaleByScheduleState(count=jnp.array(0, dtype=jnp.int32))
        opt_state += (new_state,)
        score_model = agent.score_model.replace(params=state_dict["params"], opt_state=opt_state, step=state_dict["step"])
        
        # with open(path_dir  + "target_score_model.msgpack", 'rb') as f:
        #     state_dict = flax.serialization.msgpack_restore(f.read())
        # target_score_model = agent.target_score_model.replace(params=state_dict["params"], opt_state=self.loadUtil(state_dict["opt_state"]), step=state_dict["step"])
        with open(path_dir  + "critic", 'rb') as f:
            state_dict = flax.serialization.msgpack_restore(f.read())
        critic = agent.critic.replace(params=state_dict["params"], opt_state=self.loadUtil(state_dict["opt_state"]), step=state_dict["step"])
        
        with open(path_dir  + "value", 'rb') as f:
            state_dict = flax.serialization.msgpack_restore(f.read())
        ...
        agent = agent.replace(score_model=score_model, target_score_model = score_model, critic=critic, value=value, target_critic=critic)
