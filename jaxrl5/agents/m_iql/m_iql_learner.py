...
class MIQLLearner(Agent):
    ...
    def update_v(agent, batch: DatasetDict) -> Tuple[Agent, Dict[str, float]]:
        ...
        q_values = agent.target_critic.apply_fn(
            {"params": agent.target_critic.params},
            batch["observations"],
            batch["actions"],
        )
    ...
    # 是否可以直接集成多个q网络的输出，避免过高估计
    def update_q(agent, batch: DatasetDict, expert=None) -> Tuple[Agent, Dict[str, float]]:
        ...
        def critic_loss_fn(critic_params) -> Tuple[jnp.ndarray, Dict[str, float]]:
            ...
            if expert:
                # 假设行为策略的 log 概率
                behavior_policy_log_prob = agent.actor.apply_fn({"params": agent.actor.params}, batch["observations"], training=True)# 从 batch["log_probs"] 获取或估计

                # 假设目标策略的 log 概率
                dist = agent.actor.apply_fn({"params": agent.actor.params}, batch["observations"], training=True)
                target_policy_log_prob = dist.log_prob(batch["actions"])

                # 计算重要性采样权重
                importance_weights = jnp.exp(target_policy_log_prob - behavior_policy_log_prob)
                importance_weights = jnp.clip(importance_weights, 0, 10)
            else:
                importance_weights = 1.
            # 假设 q_values 是 [q1, q2, ..., qn] 形式的数组，target_q 是目标 Q 值
            #target_q = jax.lax.stop_gradient(target_q)  # 假设 target_q 是已计算好的目标 Q 值
            num_qs = len(q_values)  # Q 网络的数量

            # 计算每个 Q 网络的损失，并对所有 Q 网络的损失取平均
            critic_loss = sum( importance_weights * (q - target_q) ** 2 for q in q_values).mean() / num_qs

            # 计算所有 Q 网络输出的均值（用于日志记录）
            q_means = [q.mean() for q in q_values]

            # 返回损失值和 Q 值的统计信息
            return critic_loss, {
                "critic_loss": critic_loss,
                "q_mean": sum(q_means) / num_qs,  # 所有 Q 网络输出的均值
                **{f"q{i+1}": q_mean for i, q_mean in enumerate(q_means)}  # 记录每个 Q 网络的均值
            }
    ...
    def update_actor(agent, batch: DatasetDict, expert = None) -> Tuple[Agent, Dict[str, float]]:
        ...
        if expert:
            # 计算行为策略的 log 概率 ，这里可以使用多个策略进行集成
            behavior_policy_log_prob = expert.actor.apply_fn({"params": expert.actor.params}, batch["observations"], training=False)# 从 batch["log_probs"] 获取或估计
            ...
            # 计算重要性采样权重
            importance_weights = jnp.exp(target_policy_log_prob - behavior_policy_log_prob)
            importance_weights = jnp.clip(importance_weights, 0, 10)  # 进行裁剪，避免过大
        else:
            importance_weights = 1.
        ...
        def actor_loss_fn(actor_params) -> Tuple[jnp.ndarray, Dict[str, float]]:
            ...
            actor_loss = -(importance_weights * exp_a * log_probs).mean() #+ bc_loss
    ...
    @jax.jit
    def update(self, batch: DatasetDict, expert=None):
